...
int currentState[3];
int lastState[] = {1,1,1}; 
int chatterTime = 50;       // after this time the bouncing problem will be solved
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(up), inc, LOW);   // call the function inc when the up pin changes value
  attachInterrupt(digitalPinToInterrupt(down), dec, LOW);
}
  ...
  for(int i=0; i<3; i++){
    ...
    if((millis() - debounceTime) > chatterTime){  //waiting for chatter time to get over
      if(currentState[i] == 0 && lastState[i] == 1){ 
        ...
        Timer1.initialize();
        Timer1.pwm(speakerPin, floor(vol*1023/5), T);
      }
      else if(currentState[i] == 1 && lastState[i] == 0) {
        Timer1.disablePwm(speakerPin);  //if unpressed 
      }
